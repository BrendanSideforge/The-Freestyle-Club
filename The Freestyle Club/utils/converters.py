...
async def get_member_info(ctx, user_arg):
    ...
    match_nickname = regex_name.match(user_arg)
    if match_mention:
...
async def get_member(ctx, user_arg):
    ...
    if match_mention:
        ...
        name2 = match_name.group(1)
        try:
            user = [x for x in server.members if x.display_name.lower() == name2.lower()][0]
        except IndexError:
            try:
                user = [x for x in server.members if x.name.lower() == name2.lower()][0]
            except IndexError:
                try:
